...
class CharacterAnalyzer:
        ...
        try:
            ...
            for encoding in encodings_to_try:
                try:
                    ...
                    print(f"Successfully loaded {len(self.df)} rows from {self.csv_file_path} using {encoding} encoding")
                    return True
            ...
            print(f"Successfully loaded {len(self.df)} rows from {self.csv_file_path} (with error handling)")
            return True
    ...
    def _filter_forbidden_digits(self) -> None:
        ...
        forbidden_messages = self.df[self.df['MESSAGE'].astype(str).str.contains('[0189]', na=False)]
        ...
        if not forbidden_messages.empty and 'DATE' in self.df.columns:
            ...
            if len(forbidden_dates) > 0:
                print(f"Found messages with forbidden digits (0,1,8,9) on dates: {', '.join(map(str, forbidden_dates))}")
        
        initial_count = len(self.df)
        self.df = self.df[~self.df['MESSAGE'].astype(str).str.contains('[0189]', na=False)]
    ...
    def _filter_problematic_chars(self) -> None:
        ...
        for column, pattern, description in filters:
            if column in self.df.columns:
                initial_count = len(self.df)
                self.df = self.df[~self.df[column].astype(str).str.contains(pattern, na=False)]
                filtered_count = initial_count - len(self.df)
                print(f"Filtered out {filtered_count} rows with {description} in {column} column")
    ...
    def preprocess_messages(self, include_first_two: bool = False, exclude_consecutive: bool = False, consecutive_count: int = 4) -> List[str]:
        ...
        for _, row in self.df.iterrows():
            ...
            if exclude_consecutive and self._has_n_consecutive_chars(message, consecutive_count):
                excluded_count += 1
                continue
            
            # Apply first two characters rule
            processed_message = message if include_first_two else (message[2:] if len(message) > 2 else "")
        ...
        if exclude_consecutive:
            print(f"Excluded {excluded_count} messages with {consecutive_count}+ consecutive identical characters")
    ...
    def _create_statistical_chart(self, chart_data: List[Tuple[str, int]], chart_title: str, 
                                 bar_color: str = '#4A9EFF', max_displayable_items: int = 50) -> None:
        """Create a standardized statistical chart with outlier detection and dark theme."""
        if len(chart_data) > max_displayable_items:
            print(f"\n(Too many items to display chart - showing all {len(chart_data)} in text format)")
            return
        ...
        chart_bars = ax.bar(range(len(chart_items)), frequency_counts, color=bar_color, alpha=0.8, edgecolor='white', linewidth=0.5)
        ...
        ax.axhline(y=counts_mean_freq, color='#FF6B6B', linestyle='--', alpha=0.9, linewidth=2,
                   label=f'Mean: {counts_mean_freq:.1f}')
        ax.axhline(y=counts_mean_freq + counts_std_freq, color='#FFD93D', linestyle=':', alpha=0.9, linewidth=2,
        ...
        legend = ax.legend(bbox_to_anchor=(0.5, -0.15), loc='upper center', ncol=3,
                          facecolor='#3d3d3d', edgecolor='white', framealpha=0.9)
        ...
        ax.set_xticklabels([f"'{chart_item}'" for chart_item in chart_display_items], 
                          rotation=x_label_rotation, color='white')
        ...
        if len(chart_data) <= 30:
            for chart_bar, frequency_count in zip(chart_bars, frequency_counts):
                ax.text(chart_bar.get_x() + chart_bar.get_width()/2, 
                       chart_bar.get_height() + max(frequency_counts) * 0.01, 
                       str(frequency_count), ha='center', va='bottom', 
    ...
    def _highlight_outliers(self, chart_bars, frequency_counts: List[int], chart_data: List[Tuple[str, int]]) -> None:
        ...
        for bar_index, (data_item, item_count) in enumerate(chart_data):
            if item_count > outlier_threshold_high or item_count < outlier_threshold_low:
    ...
    def _print_statistics(self, stats_data: List[Tuple[str, int]], stats_title: str) -> None:
        ...
        stats_outliers = [(stats_item, stats_count) for stats_item, stats_count in stats_data 
    ...
    def analyze_character_context(self, target_character: str, character_position: str) -> None:
        ...
        for processed_message in self.processed_messages:
            if pd.isna(processed_message) or not isinstance(processed_message, str) or len(processed_message) < 2:
                continue
            
            if character_position == 'after':
                for char_index in range(len(processed_message) - 1):
                    if processed_message[char_index].lower() == target_character.lower():
        ...
        # Sort by frequency
        sorted_context_chars = sorted(context_char_counts.items(), key=lambda x: x[1], reverse=True)
        ...
        # Show top results
        print(f"\nTop characters {character_position} '{target_character.upper()}' (ranked by frequency):")
        for context_rank, (context_char, context_count) in enumerate(sorted_context_chars[:20], 1):
            ...
            print(f"{context_rank:2d}. '{context_display_char}' ({context_char_type}): {context_count} times ({context_percentage:.1f}%)")
        
        # Create visualization and statistics
        context_chart_title = f"Characters Appearing {character_position.title()} '{target_character.upper()}' with Statistical Analysis"
        self._create_statistical_chart(sorted_context_chars, context_chart_title)
        self._print_statistics(sorted_context_chars, f"characters {character_position} '{target_character.upper()}'")
    ...
    def analyze_consecutive_characters(self, min_consecutive: int = 4) -> None:
        ...
        for analyzed_message in self.processed_messages:
            ...
            consecutive_sequences_found = self._find_consecutive_chars(analyzed_message, min_consecutive)
            ...
            for consecutive_char, consecutive_length in consecutive_sequences_found:
                ...
                if consecutive_char not in consecutive_char_max_lengths or consecutive_length > consecutive_char_max_lengths[consecutive_char]:
                    consecutive_char_max_lengths[consecutive_char] = consecutive_length
        
        if not consecutive_char_counts:
            print(f"No characters found with {min_consecutive}+ consecutive occurrences!")
            return
        
        print(f"\nConsecutive Character Analysis:")
        print(f"Messages with {min_consecutive}+ consecutive chars: {messages_with_consecutive_chars}/{len(self.processed_messages)}")
        print(f"Total consecutive sequences found: {total_consecutive_sequences}")
        
        # Sort by frequency
        sorted_consecutive_chars = sorted(consecutive_char_counts.items(), key=lambda x: x[1], reverse=True)
        
        print(f"\nCharacters that appear {min_consecutive}+ times consecutively (ranked by frequency):")
        for consecutive_rank, (consecutive_char, consecutive_count) in enumerate(sorted_consecutive_chars, 1):
            consecutive_max_length = consecutive_char_max_lengths[consecutive_char]
            consecutive_percentage = (consecutive_count / total_consecutive_sequences) * 100
            consecutive_char_type = self._get_char_type(consecutive_char)
            print(f"{consecutive_rank:2d}. '{consecutive_char}' ({consecutive_char_type}): {consecutive_count} sequences ({consecutive_percentage:.1f}%) - max length: {consecutive_max_length}")
        
        # Analyze by type
        self._analyze_consecutive_by_type(sorted_consecutive_chars, consecutive_char_max_lengths, min_consecutive)
        
        # Create visualization and statistics
        consecutive_chart_title = f'Frequency of Characters Appearing {min_consecutive}+ Times Consecutively with Statistical Analysis'
        self._create_statistical_chart(sorted_consecutive_chars, consecutive_chart_title)
        self._print_statistics(sorted_consecutive_chars, f"consecutive characters ({min_consecutive}+)")
    
    def _analyze_consecutive_by_type(self, sorted_consecutive_chars: List[Tuple[str, int]], 
                                   consecutive_char_max_lengths: Dict[str, int], min_consecutive: int) -> None:
        """Analyze consecutive characters by type (digits, letters, symbols)."""
        consecutive_types = {
            'DIGITS': [(consecutive_char, consecutive_count) for consecutive_char, consecutive_count in sorted_consecutive_chars if consecutive_char.isdigit()],
            'LETTERS': [(consecutive_char, consecutive_count) for consecutive_char, consecutive_count in sorted_consecutive_chars if consecutive_char.isalpha()],
            'SYMBOLS': [(consecutive_char, consecutive_count) for consecutive_char, consecutive_count in sorted_consecutive_chars if not consecutive_char.isalnum()]
        }
        ...
        for type_category_name, type_category_chars in consecutive_types.items():
            if type_category_chars:
                ...
                total_for_type_category = sum(type_count for _, type_count in type_category_chars)
                for type_rank, (type_char, type_count) in enumerate(type_category_chars[:5], 1):
                    type_max_length = consecutive_char_max_lengths[type_char]
                    type_percentage = (type_count / total_for_type_category) * 100
                    print(f"  {type_rank}. '{type_char}': {type_count} sequences ({type_percentage:.1f}%) - max length: {type_max_length}")
    ...
    def analyze_numbers(self, requested_digit_length: int) -> None:
        ...
        for processed_message in self.processed_messages:
            extracted_numbers = re.findall(r'\d+', processed_message) if isinstance(processed_message, str) else []
        ...
        filtered_number_counts = {number_string: number_count for number_string, number_count in number_frequency_counts.items() 
        ...
        range_start_num = 0 if requested_digit_length == 1 else 10 ** (requested_digit_length - 1)
        ...
        print(f"\nAnalyzing {requested_digit_length}-digit numbers ({range_start_num}-{range_end_num-1}):")
        print(f"Numbers that appear: {found_numbers_count}/{total_possible_numbers}")
        print(f"Numbers that don't appear: {total_possible_numbers - found_numbers_count}/{total_possible_numbers}")
        
        # Sort by number value for proper ordering
        sorted_numbers_by_value = sorted(filtered_number_counts.items(), key=lambda x: int(x[0]))
        
        # Show top 10 most frequent numbers
        top_frequent_numbers = sorted(filtered_number_counts.items(), key=lambda x: x[1], reverse=True)
        print(f"\nTop {min(10, len(top_frequent_numbers))} most frequent {requested_digit_length}-digit numbers:")
        for number_string, number_count in top_frequent_numbers[:10]:
            print(f"{number_string}: {number_count} occurrences")
        
        # Create visualization and statistics
        numbers_chart_title = f'Frequency of {requested_digit_length}-Digit Numbers with Statistical Analysis'
        self._create_statistical_chart(sorted_numbers_by_value, numbers_chart_title)
        self._print_statistics(sorted_numbers_by_value, f"{requested_digit_length}-digit numbers")
    ...
    def analyze_letters(self) -> None:
        ...
        alphabet_letters_az = [chr(letter_code) for letter_code in range(ord('a'), ord('z') + 1)]
        found_alphabet_letters = [(alphabet_letter, letter_frequency_counts.get(alphabet_letter, 0)) 
                                 for alphabet_letter in alphabet_letters_az if letter_frequency_counts.get(alphabet_letter, 0) > 0]
        
        total_letter_occurrences = sum(letter_count for _, letter_count in found_alphabet_letters)
        ...
        for alphabet_letter, letter_count in found_alphabet_letters:
            ...
            print(f"{alphabet_letter.upper()}: {letter_count} occurrences ({letter_percentage:.1f}%)")
        
        # Show top and bottom frequencies
        sorted_alphabet_letters = sorted(found_alphabet_letters, key=lambda x: x[1], reverse=True)
        
        print(f"\nTop 5 most frequent letters:")
        for alphabet_letter, letter_count in sorted_alphabet_letters[:5]:
            letter_percentage = (letter_count / total_letter_occurrences) * 100
            print(f"{alphabet_letter.upper()}: {letter_count} ({letter_percentage:.1f}%)")
        ...
        self._create_statistical_chart(found_alphabet_letters, letters_chart_title, bar_color='#2ED573')
    ...
    def analyze_two_char_combinations(self) -> None:
        ...
        # Generate all possible 2-character combinations (excluding 0,1,8,9 which are already filtered)
        allowed_chars = list('abcdefghijklmnopqrstuvwxyz') + ['2', '3', '4', '5', '6', '7']
        ...
        print(f"Total possible 2-character combinations: {len(all_possible_combinations)}")
        ...
        for processed_message in self.processed_messages:
            ...
            for i in range(len(processed_message) - 1):
                ...
                if (len(two_char_seq) == 2 and 
                    all(c.isalnum() for c in two_char_seq) and 
                    all(c in allowed_chars for c in two_char_seq)):
        ...
        complete_combinations = [(combo, found_combinations.get(combo, 0)) for combo in all_possible_combinations]
        found_combinations_only = [(combo, count) for combo, count in complete_combinations if count > 0]
        missing_combinations = [(combo, count) for combo, count in complete_combinations if count == 0]
        
        print(f"\nAnalyzing 2-character alphanumeric combinations:")
        print(f"Combinations that appear: {len(found_combinations_only)}/{len(all_possible_combinations)}")
        print(f"Combinations that don't appear: {len(missing_combinations)}/{len(all_possible_combinations)}")
        print(f"Total occurrences: {total_sequences}")
        
        # Sort by frequency for analysis
        sorted_combinations = sorted(found_combinations_only, key=lambda x: x[1], reverse=True)
        
        # Show top combinations
        print(f"\nTop 20 most frequent 2-character combinations:")
        for rank, (combo, count) in enumerate(sorted_combinations[:20], 1):
            percentage = (count / total_sequences) * 100 if total_sequences > 0 else 0
            print(f"{rank:2d}. '{combo.upper()}': {count} occurrences ({percentage:.2f}%)")
        
        # Show bottom combinations (that appear)
        if len(sorted_combinations) > 20:
            print(f"\nBottom 10 least frequent combinations (that appear):")
            for rank, (combo, count) in enumerate(sorted_combinations[-10:], len(sorted_combinations)-9):
                percentage = (count / total_sequences) * 100 if total_sequences > 0 else 0
                print(f"{rank:2d}. '{combo.upper()}': {count} occurrences ({percentage:.2f}%)")
        ...
        combinations_chart_title = '2-Character Alphanumeric Combination Frequencies with Statistical Analysis'
        if len(found_combinations_only) <= 100:  # Only chart if reasonable number
            self._create_statistical_chart(sorted_combinations, combinations_chart_title, bar_color='#9B59B6')
        else:
            print(f"\n(Too many combinations ({len(found_combinations_only)}) to display chart - showing statistics only)")
        
        self._print_statistics(found_combinations_only, "2-character combinations")
    
    def _analyze_combinations_by_type(self, sorted_combinations: List[Tuple[str, int]], total_sequences: int) -> None:
        """Analyze 2-character combinations by type categories."""
        combination_types = {
            'LETTER-LETTER': [(combo, count) for combo, count in sorted_combinations if combo[0].isalpha() and combo[1].isalpha()],
            'DIGIT-DIGIT': [(combo, count) for combo, count in sorted_combinations if combo[0].isdigit() and combo[1].isdigit()],
            'LETTER-DIGIT': [(combo, count) for combo, count in sorted_combinations if combo[0].isalpha() and combo[1].isdigit()],
            'DIGIT-LETTER': [(combo, count) for combo, count in sorted_combinations if combo[0].isdigit() and combo[1].isalpha()]
        }
        ...
        for type_name, type_combos in combination_types.items():
            if type_combos:
                ...
                type_percentage = (type_total / total_sequences) * 100 if total_sequences > 0 else 0
                print(f"\n{type_name}: {len(type_combos)} different combinations, {type_total} total occurrences ({type_percentage:.2f}%)")
                
                # Show top 5 for each type
                for rank, (combo, count) in enumerate(type_combos[:5], 1):
                    combo_percentage = (count / type_total) * 100 if type_total > 0 else 0
                    print(f"  {rank}. '{combo.upper()}': {count} ({combo_percentage:.1f}% of {type_name.lower()})")
    ...
    def run_interactive_analysis(self) -> None:
        ...
        print("Enter a NUMBER (1, 2, 3, etc.) to analyze digit sequences of that length")
        print("Enter any LETTER to analyze alphabet frequencies (a-z)")
        print("Enter any 2-CHARACTER combination (AB, 23, X7, etc.) to analyze all 2-char combinations")
        print("Enter ASTERISKS (**,***,****,etc.) to analyze characters appearing that many times consecutively")
        print("Enter 'X_' (where X is any letter/number) to see what characters appear AFTER X")
        print("Enter '_X' (where X is any letter/number) to see what characters appear BEFORE X")
        print("Add '+' to INCLUDE the first 2 characters (e.g., '2+' or 'A+')")
        print("Add '-' to EXCLUDE strings with consecutive identical characters (e.g., '2-')")
        print("You can combine modifiers (e.g., '2+-')")
        print("Without modifiers, the first 2 characters are ignored and consecutive chars are included")
        ...
        while attempts < max_attempts:
            try:
                ...
                if core_analysis_input and all(c == '*' for c in core_analysis_input) and len(core_analysis_input) >= 2:
                    consecutive_count = len(core_analysis_input)
                    # For exclusion filter, we need to pass the consecutive count
                    self.preprocess_messages(include_first_two_chars, exclude_consecutive_chars, consecutive_count)
                ...
                self.preprocess_messages(include_first_two_chars, exclude_consecutive_chars)
                
                # Handle different analysis types
                if self._handle_pattern_analysis(core_analysis_input):
                    break
                elif len(core_analysis_input) == 2 and all(c.isalnum() for c in core_analysis_input):
