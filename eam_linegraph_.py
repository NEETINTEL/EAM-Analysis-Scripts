...
PREFIX_GROUPS = {
    'Group 1 (Green)': {
        'prefixes': ['YL','YT'],
        'base_hex': '#06b050', 'marker': 'o', 'hatch': None
    },
    'Group 2 (Orange)': {
        'prefixes': ['T5','F6'],
        'base_hex': '#ff9000', 'marker': 's', 'hatch': '//'
    },
    'Group 3 (Yellow)': {
        'prefixes': [],
        'base_hex': '#ffff00', 'marker': '^', 'hatch': '\\\\'
    },
    'Group 4 (Pink)': {
        'prefixes': ['6K','JC'],
        'base_hex': '#ff0467', 'marker': '*', 'hatch': '.....'
    },
    'Uncategorized': {
        'prefixes': [], 'base_hex': '#121930', 'marker': '.', 'hatch': None
    }
}

VALID_MARKERS = {'.', ',', 'o', 'v', '^', '<', '>', '1', '2', '3', '4', '8', 's', 'p', 'P', '*', 'h', 'H', '+', 'x', 'X', 'D', 'd', '|', '_'}
...
class DateProcessor:
    ...
    def parse_compact_date(date_string):
        ...
        if len(date_string) == 2:
            ...
            return 2000 + int(date_string[:2]), int(date_string[2:4]), int(date_string[4:])
        else:
    ...
    def format_date_label(date_obj, label_type='chart', bw_mode=False):
        ...
        if label_type == 'yaxis':
            ...
            return Text(text=f"{day_abbr} {date_formatted}", fontweight='bold', color=text_color)
        else:  # chart
            day_abbr = date_obj.strftime('%a').upper()
...
class DataProcessor:
    ...
    def process_datetime_columns(df_input):
        ...
        df_processed['DateTime_combined'] = df_processed['Date_str'] + ' ' + df_processed['Time_str']
        ...
        # Apply safe parsing
        df_processed['Datetime'] = df_processed['DateTime_combined'].apply(safe_datetime_parse)
        ...
        if invalid_count > 0:
            print(f"Warning: Found {invalid_count} rows with invalid DATE/UTC entries - these will be skipped.")
        ...
        if len(df_processed) > 0:
            ...
            df_processed['Time_minutes'] = df_processed['Datetime'].dt.hour * 60 + df_processed['Datetime'].dt.minute
            ...
            return pd.DataFrame(columns=['Date', 'Time', 'Callsign', 'Prefix', 'Q', 'Message', 
                                       'Datetime', 'Time_minutes', 'DateOnly', 'Year', 'Month'])
        
        # Clean up temporary columns
        df_processed = df_processed.drop(['Date_str', 'Time_str', 'DateTime_combined'], axis=1, errors='ignore')
        
        final_count = len(df_processed)
        if invalid_count > 0:
            print(f"Successfully processed {final_count} rows (skipped {invalid_count} invalid entries).")
        
        return df_processed
    
    @staticmethod
    def apply_filtering(df_input, exclude_percent=True, exclude_backslash_entries=False, exclude_duplicate_entries=False):
        ...
        if exclude_backslash_entries:
            backslash_mask = ~df_filtered['Q'].astype(str).str.contains('\\\\', na=False)
            df_filtered = df_filtered[backslash_mask].copy()
        
        # Filter duplicates
        if exclude_duplicate_entries:
            df_filtered['CallsignMessage'] = df_filtered['Callsign'].astype(str) + '|' + df_filtered['Message'].astype(str)
            df_filtered['DuplicateKey'] = df_filtered['DateOnly'].astype(str) + '|' + df_filtered['CallsignMessage']
            duplicate_mask = df_filtered.duplicated(subset=['DuplicateKey'], keep='first')
            df_filtered = df_filtered[~duplicate_mask].copy()
            df_filtered = df_filtered.drop(['CallsignMessage', 'DuplicateKey'], axis=1, errors='ignore')
...
class StyleProcessor:
    ...
    def setup_prefix_grouping(df_data, is_short_period, is_intermediate_period=False):
        ...
        for group_name, group_info in PREFIX_GROUPS.items():
            if group_name == 'Uncategorized':
                ...
                candidate_prefixes = [p for p in all_prefixes if p not in group_to_prefix]
                if candidate_prefixes:
                    group_prefixes = StyleProcessor.get_sorted_prefixes_by_recency(candidate_prefixes, df_data)
                ...
                candidate_prefixes = [p for p in group_info['prefixes'] if p in all_prefixes]
                if candidate_prefixes:
                    group_prefixes = StyleProcessor.get_sorted_prefixes_by_recency(candidate_prefixes, df_data)
                    for prefix in group_prefixes:
                        group_to_prefix[prefix] = group_name
            ...
            group_prefixes_final = [p for p in all_prefixes if group_to_prefix.get(p) == group_name]
            if not group_prefixes_final:
                continue
                
            # Sort final prefixes for consistent alpha assignment
            group_prefixes_final = StyleProcessor.get_sorted_prefixes_by_recency(group_prefixes_final, df_data)
    ...
    @staticmethod
    def create_legend_handles(all_prefixes, group_to_prefix_mapping, daily_counts=None, bw_mode=False, is_bar_chart=False, df_data=None):
        ...
        for group_name, info in PREFIX_GROUPS.items():
            ...
            if df_data is not None and candidate_prefixes:
                group_prefixes_for_legend = StyleProcessor.get_sorted_prefixes_by_recency(candidate_prefixes, df_data)
            ...
            if is_bar_chart and daily_counts is not None:
                group_prefixes_with_data = [
                    p for p in group_prefixes_for_legend 
                    if p in daily_counts.columns and daily_counts[p].sum() > 0 and '%' not in str(p)
                ]
            else:
                group_prefixes_with_data = [p for p in group_prefixes_for_legend if '%' not in str(p)]
            ...
            legend_handles.append(
                Line2D([], [], marker=legend_marker, linestyle='', 
                       color=info['base_hex'], label=label_text,
...
class PlotCreator:
    ...
    @staticmethod
    def create_timeline_plot(df_data, dates_sorted, date_to_y, group_to_prefix_mapping, 
                           prefix_color_mapping, prefix_alpha_timeline_mapping, is_short_period, 
                           bw_mode, period_start, period_end, include_percent, secret_marker=None, is_intermediate_period=False):
        ...
        for group_name, group_info in PREFIX_GROUPS.items():
            ...
            for prefix in group_prefixes:
                ...
                if bw_mode:
                    plt.scatter(
                        subset_data['Time_minutes'], subset_data['y'],
                        color=prefix_color_mapping[prefix], s=150, 
                        alpha=prefix_alpha_timeline_mapping[prefix],
                        marker=marker_shape, edgecolors='black', linewidths=0.6
                    )
                else:
                    plt.scatter(
                        subset_data['Time_minutes'], subset_data['y'],
                        color=prefix_color_mapping[prefix], s=50, 
                        alpha=prefix_alpha_timeline_mapping[prefix],
                        marker=marker_shape
                    )

        PlotCreator._setup_timeline_styling(dates_sorted, date_to_y, is_short_period, 
                                          period_start, period_end, include_percent, bw_mode, is_intermediate_period)
        
        # Add legend with BW mode styling
        legend_handles = StyleProcessor.create_legend_handles(all_prefixes, group_to_prefix_mapping, bw_mode=bw_mode, df_data=df_data)
        ...
        plt.legend(handles=legend_handles, title='Groups', bbox_to_anchor=(1.05, 1),
                   loc='upper left', facecolor=legend_face_color, edgecolor=legend_edge_color,
                   labelcolor=legend_label_color, title_fontsize=12, fontsize=10)
        
        plt.tight_layout()
        plt.show()
    
    @staticmethod
    def create_bar_chart(df_data, complete_dates, group_to_prefix_mapping, prefix_color_mapping, 
                        prefix_alpha_bar_mapping, is_short_period, bw_mode, period_start, 
                        period_end, include_percent, is_intermediate_period=False):
        """Create the stacked bar chart"""
        apply_plot_style(bw_mode)
        
        # Prepare data efficiently
        if len(df_data) > 0:
            daily_counts_raw = df_data.groupby(['Date', 'Prefix']).size().unstack(fill_value=0)
        ...
        for group_name, group_info in PREFIX_GROUPS.items():
            candidate_prefixes = [p for p in group_info['prefixes'] if p in all_prefixes_in_data]
            ...
            for prefix in group_prefixes:
                ...
                face_color = mcolors.to_rgba(prefix_color_mapping[prefix], alpha=prefix_alpha_bar_mapping[prefix])
                ...
                plt.bar(range(len(daily_counts)), values, bottom=bottom_values, **bar_kwargs)
                bottom_values = [b + v for b, v in zip(bottom_values, values)]

        PlotCreator._setup_bar_chart_styling(daily_counts, is_short_period, period_start, 
                                           period_end, include_percent, bw_mode, complete_dates, is_intermediate_period)
        
        # Add legend with BW mode styling
        legend_handles = StyleProcessor.create_legend_handles(all_prefixes_in_data, group_to_prefix_mapping, 
                                                             daily_counts, bw_mode, is_bar_chart=True, df_data=df_data)
        legend_face_color = 'white' if bw_mode else 'black'
        legend_edge_color = 'black' if bw_mode else 'white'
        legend_label_color = 'black' if bw_mode else 'white'
        
        plt.legend(handles=legend_handles, title='Groups', bbox_to_anchor=(1.05, 1), loc='upper left',
                   facecolor=legend_face_color, edgecolor=legend_edge_color, labelcolor=legend_label_color, 
    ...
    @staticmethod
    def _setup_timeline_styling(dates_sorted, date_to_y, is_short_period, period_start, 
                              period_end, include_percent, bw_mode, is_intermediate_period=False):
        ...
        if is_intermediate_period:
            ...
            week_boundaries = DateProcessor.get_week_boundaries(period_start, period_end)
            y_positions = []
            y_labels = []
            ...
            for boundary_date, week_label in week_boundaries:
                if boundary_date in date_to_y:
                    ...
                    y_labels.append(Text(text=week_label, fontweight='bold', color=text_color))
            ...
            month_boundaries = DateProcessor.get_month_boundaries(period_start, period_end)
            y_positions = []
            y_labels = []
            ...
            for boundary_date in month_boundaries:
                if boundary_date in date_to_y:
                    ...
                    y_labels.append(Text(text=month_label, fontweight='bold', color=text_color))
            ...
            y_tick_labels = [DateProcessor.format_date_label(d, 'yaxis', bw_mode) for d in dates_sorted[::-1]]
        ...
        plt.xlim(-30, 1470)
        plt.xticks(ticks=range(0, 1441, 60), labels=[f'{h:02d}:00' for h in range(0, 25, 1)], color=text_color)
    ...
    @staticmethod
    def _setup_bar_chart_styling(daily_counts, is_short_period, period_start, period_end, 
                               include_percent, bw_mode, complete_dates, is_intermediate_period=False):
        """Setup styling for bar chart"""
        text_color = 'black' if bw_mode else 'white'
        grid_color = 'gray' if bw_mode else 'grey'
        
        plt.xlabel('Date', color=text_color, fontsize=12, fontweight='bold')
        plt.ylabel('Number of Messages', color=text_color, fontsize=12, fontweight='bold')
        ...
        if is_intermediate_period:
            ...
            date_labels_chart = [DateProcessor.format_date_label(date_val, 'chart', bw_mode) for date_val in daily_counts.index]
...
class UserInterface:
    ...
    def parse_user_input(user_input_str):
        ...
        if len(choice_parts) == 2:
            ...
            if potential_marker in VALID_MARKERS:
                try:
                    ...
                    return "", False, False, secret_marker_found, initial_date_found, False
                except ValueError:
                    pass

        # Check for marker with exclamation mark only (e.g., "d!")
        if len(user_input_str) == 2 and user_input_str.endswith('!') and user_input_str[0] in VALID_MARKERS:
        ...
        return user_input_str, include_percent_flag, bw_mode_flag, secret_marker_found, initial_date_found, use_detailed_flag

def process_date_range_input(user_date_input, main_df, latest_year, latest_month, 
                           include_percent_flag, exclude_backslashes_flag, 
                           exclude_duplicates_flag, df_raw):
    ...
    if '%' in user_date_input and not include_percent_flag:
        df_with_percent = df_raw[['Date', 'Time', 'Callsign', 'Prefix', 'Q', 'Message']].copy()
        df_with_percent = DataProcessor.process_datetime_columns(df_with_percent)
        main_df = DataProcessor.apply_filtering(df_with_percent, exclude_percent=False, 
    ...
    if len(input_parts) > 0:
        ...
        if len(last_part) == 2 and last_part.endswith('!') and last_part[0] in VALID_MARKERS:
    ...
    if not user_date_input:
        df_filtered = main_df[(main_df['Year'] == latest_year) & (main_df['Month'] == latest_month)].copy()
        period_start_date = pd.to_datetime(f"{latest_year}-{latest_month}-01").date()
        period_end_date = (pd.to_datetime(f"{latest_year}-{latest_month}-01") + pd.offsets.MonthEnd(0)).date()
        date_description = pd.to_datetime(f"{latest_year}-{latest_month}-01").strftime('%B %Y')
    else:
        input_parts = user_date_input.split()
        
        if len(input_parts) == 1:
            selected_year, selected_month, selected_day = DateProcessor.parse_compact_date(input_parts[0])
            ...
            if selected_month is None:
                ...
                df_filtered = main_df[(main_df['Year'] == selected_year) & (main_df['Month'] == selected_month)].copy()
                period_start_date = pd.to_datetime(f"{selected_year}-{selected_month}-01").date()
                period_end_date = (pd.to_datetime(f"{selected_year}-{selected_month}-01") + pd.offsets.MonthEnd(0)).date()
                date_description = pd.to_datetime(f"{selected_year}-{selected_month}-01").strftime('%B %Y')
            else:
                target_date = pd.to_datetime(f"{selected_year}-{selected_month:02d}-{selected_day:02d}")
                df_filtered = main_df[main_df['Datetime'].dt.date == target_date.date()].copy()
                period_start_date = target_date.date()
                period_end_date = target_date.date()
                date_description = target_date.strftime('%B %d, %Y')
                
        elif len(input_parts) == 2:
            start_year, start_month, start_day = DateProcessor.parse_compact_date(input_parts[0])
            end_year, end_month, end_day = DateProcessor.parse_compact_date(input_parts[1])
            
            if start_day is not None:
                period_start_date = pd.to_datetime(f"{start_year}-{start_month:02d}-{start_day:02d}").date()
            elif start_month is not None:
                period_start_date = pd.to_datetime(f"{start_year}-{start_month:02d}-01").date()
            else:
                period_start_date = pd.to_datetime(f"{start_year}-01-01").date()
            
            if end_day is not None:
                period_end_date = pd.to_datetime(f"{end_year}-{end_month:02d}-{end_day:02d}").date()
            elif end_month is not None:
                period_end_date = (pd.to_datetime(f"{end_year}-{end_month:02d}-01") + pd.offsets.MonthEnd(0)).date()
            else:
                period_end_date = pd.to_datetime(f"{end_year}-12-31").date()
            
            df_filtered = main_df[(main_df['Datetime'].dt.date >= period_start_date) & 
                           (main_df['Datetime'].dt.date <= period_end_date)].copy()
            ...
            if period_start_date == period_end_date:
                ...
                if period_start_date.month == period_end_date.month:
                    date_description = f"{period_start_date.strftime('%B %d')} - {period_end_date.strftime('%d, %Y')}"
                else:
                    date_description = f"{period_start_date.strftime('%B %d')} - {period_end_date.strftime('%B %d, %Y')}"
            else:
                date_description = f"{period_start_date.strftime('%B %d, %Y')} - {period_end_date.strftime('%B %d, %Y')}"
    
    return df_filtered, period_start_date, period_end_date, date_description, secret_marker_detected, include_percent_flag
    ...
    try:
        ...
        if missing_cols:
            ...
            for missing_col in missing_cols:
                while True:
                    user_input = input(f"Enter column name or number for {missing_col}: ").strip()
...
if include_percent_flag or bw_mode_flag or secret_marker_detected or initial_date_detected:
    if secret_marker_detected:
        print(f"\nSpecial marker input detected: '{secret_marker_detected}' will be used for timeline plots.")
...
if parsed_input == "":
    print(f"\nExpress mode activated{' with special options' if any([include_percent_flag, bw_mode_flag, secret_marker_detected, initial_date_detected]) else ''}.")
else:
...
if use_detailed_options_flag:
    ...
    if backslash_count > 0:
        ...
        exclude_choice = input("Input NULL to include these lines, or input X to exclude them.\nInput: ").strip().lower()
    ...
    main_df_temp['CallsignMessage'] = main_df_temp['Callsign'].astype(str) + '|' + main_df_temp['Message'].astype(str)
    main_df_temp['DuplicateKey'] = main_df_temp['DateOnly'].astype(str) + '|' + main_df_temp['CallsignMessage']
    duplicate_count = main_df_temp.duplicated(subset=['DuplicateKey'], keep='first').sum()
    
    if duplicate_count > 0:
        print(f"\nFound {duplicate_count} lines with duplicate messages.")
        print(f"This represents {(duplicate_count/len(main_df))*100:.1f}% of the current data.")
        exclude_duplicates_choice = input("Input NULL to include these lines, or input X to exclude them.\nInput: ").strip().lower()
        exclude_duplicates_flag = exclude_duplicates_choice == 'x'
        if exclude_duplicates_flag:
            print(f"{duplicate_count} lines excluded.")

# Apply all filtering
main_df = DataProcessor.apply_filtering(main_df, exclude_percent=not include_percent_flag, 
                    exclude_backslash_entries=exclude_backslashes_flag, 
                    exclude_duplicate_entries=exclude_duplicates_flag)
...
try:
    available_months = main_df.groupby(['Year', 'Month']).size().reset_index(name='count').sort_values(['Year', 'Month'])
    latest_year = available_months['Year'].max()
    latest_month = available_months[available_months['Year'] == latest_year]['Month'].max()
except Exception as e:
...
if initial_date_detected:
    try:
        selected_year, selected_month, selected_day = DateProcessor.parse_compact_date(initial_date_detected)
        if selected_month is None:
            period_start_date = pd.to_datetime(f"{selected_year}-01-01").date()
            period_end_date = pd.to_datetime(f"{selected_year}-12-31").date()
        elif selected_day is None:
...
if use_detailed_options_flag:
    ...
    for _, row in available_months.iterrows():
        month_abbr = pd.to_datetime(f"{row['Year']}-{row['Month']}-01").strftime('%b').upper()
        marker_text = " (LATEST)" if (row['Year'] == latest_year and row['Month'] == latest_month) else ""
        print(f"{row['Year']} {row['Month']:02d} [{month_abbr}]: {row['count']:>3} messages{marker_text}")
    ...
    while True:
        ...
        try:
            df_filtered, period_start_date, period_end_date, date_description, secret_marker_detected, include_percent_flag = process_date_range_input(
                user_date_input, main_df, latest_year, latest_month, 
                include_percent_flag, exclude_backslashes_flag, 
                exclude_duplicates_flag, df_raw)
...
complete_date_range = pd.date_range(start=period_start_date, end=period_end_date, freq='D')
...
is_intermediate_period = DateProcessor.is_intermediate_period(period_start_date, period_end_date)
...
group_to_prefix_mapping, prefix_color_mapping, prefix_alpha_timeline_mapping, prefix_alpha_bar_mapping = StyleProcessor.setup_prefix_grouping(
    main_df, is_short_period, is_intermediate_period)

# Step 8: Create plots
period_type = 'Short' if is_short_period else ('Intermediate' if is_intermediate_period else 'Long')
print(f"Period type: {period_type} ({(period_end_date - period_start_date).days + 1} days)\n\nReports are now being generated.")

# Timeline scatter plot
PlotCreator.create_timeline_plot(main_df, dates_sorted, date_to_y_mapping, group_to_prefix_mapping, prefix_color_mapping, 
                    prefix_alpha_timeline_mapping, is_short_period, 
                    bw_mode_flag, period_start_date, period_end_date, 
                    include_percent_flag, secret_marker_detected, is_intermediate_period)
print("\nReport (type 1) generated successfully.")

# Stacked bar chart
PlotCreator.create_bar_chart(main_df, complete_dates, group_to_prefix_mapping, prefix_color_mapping, 
                prefix_alpha_bar_mapping, is_short_period, 
                bw_mode_flag, period_start_date, period_end_date, include_percent_flag, is_intermediate_period)
