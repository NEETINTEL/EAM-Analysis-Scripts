...
def read_csv_data(filename):
    ...
    try:
        ...
        df_filtered = df_filtered[df_filtered['PR'].astype(str).str.match(r'^[A-Za-z0-9]+$', na=False)]
...
def display_recent_prefixes(prefix_stats):
    ...
    for i, stats in enumerate(prefix_stats, 1):
        interval = f"{stats['min_date_str']} — {stats['max_date_str']}" if stats['min_date_str'] != stats['max_date_str'] else stats['max_date_str']
        length_range = f"{stats['min_length']}-{stats['max_length']}" if stats['min_length'] != stats['max_length'] else str(stats['min_length'])
        
        print(f"{i:2d}. {stats['prefix']:<3} (interval: {interval:<15}, messages: {stats['message_count']:>3}, message lengths: {length_range})")
    ...
    for i in range(len(cleaned_messages)):
        ...
        for j in range(i + 1, len(cleaned_messages)):
            ...
            if offset is not None:
                ...
                offsets = list(range(-(len_msg - 1), max_len))  # Sliding across all offsets
...
def check_existing_files(prefix, current_date):
    ...
    for file in existing_files:
        ...
        match = re.search(rf'eam_slider_output_{re.escape(prefix)}_(\d{{6}})\.txt', file)
        if match:
    ...
    if newer_files:
        ...
        response = input(f"Current analysis is for {current_date_str}. Continue anyway? (y/N): ").strip().lower()
        if response != 'y':
...
def write_results_to_file(results, prefix, latest_date):
    ...
    with open(filename, "w", encoding="utf-8") as file:
        file.write(f"Best Match Results for Prefix '{prefix}' (Sorted by Total Matches):\n")
        file.write(f"Total comparisons with {len(sorted_results)} matches above threshold:\n\n")
        
        for result in sorted_results:
            base_serial, compared_serial, best_alignment, best_offset, max_matches = result[:5]
            if len(result) >= 9:  # Check if date/time info is available
                ...
                base_time_str = str(base_time).zfill(4)  # Ensure 4 digits with leading zeros
                ...
                file.write(f"Base Message: {base_serial} [{base_date_str} {base_time_formatted}], Compared with: {compared_serial} [{msg_date_str} {msg_time_formatted}]\n")
            else:
                file.write(f"Base Message: {base_serial}, Compared with: {compared_serial}\n")
...
def main():
    ...
    # Get user selection
    while True:
        user_input = input("\nEnter a prefix to analyze (or press Enter to exit): ").strip().upper()
        ...
        for _, row in prefix_data.iterrows():
            message = row['MESSAGE']
            date = row['DATE']
            time = row.get('UTC', '0000')  # Default to '0000' if UTC column doesn't exist
        ...
        best_match_results = slide_compare_multiple_grouped(unique_message_data, offset=offset_value, min_matches=min_matches_value)
        ...
        if best_match_results:
            ...
            output_filename = write_results_to_file(best_match_results, user_input, latest_date)
            print(f"Results with at least {min_matches_value} matches have been written to {output_filename}")
            print(f"Found {len(best_match_results)} message pairs with sufficient matches.")
            
            # Display results to console
            print(f"\n--- Results for Prefix '{user_input}' ---")
            sorted_results = sorted(best_match_results, key=lambda x: x[4], reverse=True)
            ...
            for result in sorted_results:
                ...
                if len(result) >= 9:  # Check if date/time info is available
                    ...
                    print(f"\nBase: {base_serial} [{base_date_str} {base_time_formatted}], Compared: {compared_serial} [{msg_date_str} {msg_time_formatted}]")
                else:
